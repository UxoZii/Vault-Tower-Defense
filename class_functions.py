...
def tower_shoot(self):
    ...
    # Cat bonuses
    if self.id == 0:
        self.currentDamage += self.upgrade1 * 5
        if self.upgrade1 == 5:
    ...
    if self.id == 5:
        ...
        self.currentAttackSpeed -= self.upgrade1 * 8
        if self.upgrade1 == 5:
    ...
    if self.id == 11:
        self.currentDamage += int(self.pops) / 100 * (self.upgrade1 * 10)
        self.currentRange += self.upgrade3 * 50
        self.currentAttackSpeed -= self.upgrade2 * 2
    ...
    if self.timer >= self.currentAttackSpeed:
        ...
        if self.id == 1:
            ...
            test = functions.is_bloon_in_range(self)
            if test is not False:
                functions.damage_bloon(test, classes.Projectile(0, 0, self.currentDamage, 0, data.projectiles_png[self.id], 0, self))

        elif self.id == 8:
            test = functions.is_bloon_in_range(self)
            if test is not False:
                test.abnor = True
                test.abnorTimer = self.projectileSpeed
