...
def draw():
    ...
    if data.show_tower_menu:
        ...
        draw_text(int(data.tower_stats[data.selectedTower.id][0] / 2),58,600)
...
def damage_bloon(bloon, attack):
    ...
    if bloon.hp <= 0:
        ...
        del bloon
        if attack.owner.id == 5:
            if attack in data.projectiles:
                data.projectiles.remove(attack)
                del attack
                return
        return
    bloon_list = []
